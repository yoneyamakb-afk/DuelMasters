...
{
  // 旧テストの呼び方を v15 に橋渡し：string→enum 変換 & 戻りを MinimalState に統一
  public static class EngineCompatExtensions
  {
    // Step(string,string) — レシーバ型を問わず拾う
    public static MinimalState Step(this object adapter, IGameState state, string phase, string priority)
      => InvokeStep(adapter, state, Parse<Phase>(phase), Parse<Priority>(priority));

    // Step(string)
    public static MinimalState Step(this object adapter, IGameState state, string phase)
      => InvokeStep(adapter, state, Parse<Phase>(phase), null);

    // ApplyReplacement(state, 任意) — Dictionary 等も吸収
    ...
    private static MinimalState ToMinimal(IGameState gs)
    {
      ...
      catch
      {
        ...
        return (MinimalState)(object)gs; // 実体が MinimalState の場合のみ最終手段
      }
    }
  }
}
